...
app
		.controller(
				...
				function($scope, $rootScope, $filter, $http, $location,
					...
					$scope.toggleRight = buildToggler1('right');
					
					$scope.previewAsset = function(assetForPreview){
						console.log("Inside previewAsset 1111 ::"+assetForPreview.name.val);
						console.log("assetForPreview.name.id ::"+assetForPreview.name.id);
						// alert("Inside previewAsset new
						// ::"+assetForPreview.name.val);
						$scope.assetForPreview = assetForPreview;
						$scope.date = new Date(assetForPreview.modified);
						console.log("NEW DATE");
						// $scope.date = ($scope.date).format("m/dd/yy");
						...
						// buildToggler('right');
						/*
						 * var method = "GET"; var relationsUrl =
						 * "/Glossary/relations"; var rolesUrl =
						 * "/Glossary/roles"; var commentsUrl =
						 * "/Glossary/commentList"; // Request to GET relations
						 * $http({ method : method, url : relationsUrl, data :
						 * angular.toJson($scope.searchForm), params : { term :
						 * assetForPreview.name.id, dgcRestUrl :
						 * $scope.dgcRestUrl, proxyUrl : $scope.proxyUrl,
						 * proxyPort : $scope.proxyPort, apiUser :
						 * $scope.apiUser, apiUserPassword :
						 * $scope.apiUserPassword }, headers : { 'Content-Type' :
						 * 'application/json' } }).then(function
						 * successCallback(response) { $scope.relationList =
						 * response.data; $scope.divlocation = index; },
						 * function errorCallback(response) {
						 * console.log(response.statusText); }); // Request to
						 * GET roles $http({ method : method, url : rolesUrl,
						 * data : angular.toJson($scope.searchForm), params : {
						 * term : assetForPreview.name.id, dgcRestUrl :
						 * $scope.dgcRestUrl, proxyUrl : $scope.proxyUrl,
						 * proxyPort : $scope.proxyPort, apiUser :
						 * $scope.apiUser, apiUserPassword :
						 * $scope.apiUserPassword }, headers : { 'Content-Type' :
						 * 'application/json' } }).then(function
						 * successCallback(response) { $scope.memberList =
						 * response.data; $scope.loadingGroup = false; },
						 * function errorCallback(response) {
						 * console.log(response.statusText); }); // Request to
						 * GET comments $http({ method : method, url :
						 * commentsUrl, data :
						 * angular.toJson($scope.searchForm), params : { term :
						 * assetForPreview.name.id, dgcRestUrl :
						 * $scope.dgcRestUrl, proxyUrl : $scope.proxyUrl,
						 * proxyPort : $scope.proxyPort, apiUser :
						 * $scope.apiUser, apiUserPassword :
						 * $scope.apiUserPassword, commentsApi1 :
						 * $scope.commentsApi1, commentsApi2 :
						 * $scope.commentsApi2 }, headers : { 'Content-Type' :
						 * 'application/json' } }).then(function
						 * successCallback(response) { $scope.commentsList =
					 ...
					 $scope.isOpenRight = function(){
						 // console.log("contentWidth isOpen::"+
							// $scope.contentWidth);
						  $scope.contentWidth = "width:100%;";
						// console.log("contentWidth isOpen AFTER::"+
						// $scope.contentWidth);
					...
					/* Functions for tioggling Side Nav bars */
					...
					/*
					 * function buildToggler(navID) { return function() { //
					 * Component lookup should always be available since we are
					 * not using `ng-if` //alert("Sai");
					 * console.log("contentWidth
					 * initially"+$scope.contentWidth); $scope.contentWidth =
					 * "width:calc(100% - 600px);"; console.log("SAI
					 * NARESH"+navID); console.log("contentWidth
					 * "+$scope.contentWidth); // alert("toggle"+navID);
					 * $mdSidenav(navID) .toggle() .then(function () {
					 * $log.debug("toggle3 " + navID + " is done"); }); }; }
					 */
					
					function buildToggler1(componentId) {
					      return function() {
					...
					$scope.init = function() {
						...
						location.search.substr(1).split("&").forEach(
								function(item) {
									tmp = item.split("=");
									if (tmp[0] === "term") {
										$scope.searchTerm = decodeURI(tmp[1]);
									}
									if (tmp[0] === "showSearch") {
										$scope.showSearch = tmp[1];
									}
									if (tmp[0] === "user") {
										$scope.userName = tmp[1];
									}
									/*if (tmp[0] === "app") {
										app = tmp[1];
										$scope.app = app;
						...
						$http({
							...
							url : './appConfigLatest.json'
								...
								.then(function successCallback(response) {
											...
											angular.forEach($scope.config.apis,function(val, key) {
											...
											angular.forEach($scope.config.application,
															function(value, key) {
																console.log("application details value:"+ value.value+ " and Key :"+ key+ " and default app is:"+ $scope.defaultApp);
																if (key == $scope.defaultApp) {
																	angular.forEach(value,function(val,key) {
																						console.log("KEY:::"+ key);
											...
											$http(
													...
													.then(
															function successCallback(
																	response) {
																searchStatus = "true";
																console.log("DOMAIN LENGTH 1:"+ response.data.length);
											...
											$http(
													...
													.then(
															function successCallback(
																...
																console.log("SEARCH LENGTH 1:"+ response.data);
																$scope.glossaryList = response.data[1];
																$scope.glossaryListSize = response.data[0];
																console.log("SEARCH  glossaryList:"+ $scope.glossaryList);
																console.log("SEARCH  glossaryListSize:"+ $scope.glossaryListSize);
																// $scope.loading
																// = false;
					...
					$scope.loadApp = function (selectedApp){
						alert("Inside loadApp::"+selectedApp);
						console.log("***********************************************Inside Load App***********************************************");
						angular.forEach($scope.config.application,
								function(value, key) {
									console.log("application details Key :"+ key+ " and app is:"+ app);
									if (key == "ReportsApp") {
										angular.forEach(value,function(val,key) {
															console.log("KEY:::"+ key);
															if (key == "searchCategory") {
								...
								console.log("***********************************************END Inside Load App***********************************************");
								});
